...
public class Fire : Interactable
{
    ...
    void Start()
    {
        ...
        fireSizzle = Resources.Load<AudioClip>(Path.Combine(FileUtilities.SOUNDS, "fire_sizzle"));
    ...
    protected override void OnInteract(BlobController blob)
    {
        ...
        if (blob.Material.HasAll(BlobMaterialProperties.Can_Ignite)) {
            Ignite();
            interacted = true;
        }
        else if (blob.Material.HasAll(BlobMaterialProperties.Can_Extinguish)) 
        {
            Extinguish();
            interacted = true;
        }
